...
def load_frame(path):
    arr = np.load(path)
    if arr.ndim == 3 and arr.shape[:2] == (DEPTH, HEIGHT):
        if arr.shape[2] < WIDTH:
            raise ValueError(f'Unexpected shape for {path}: {arr.shape}')
        vol = arr[..., :WIDTH].astype(np.float32)
    elif arr.ndim == 2 and arr.shape[0] == DEPTH * HEIGHT:
        if arr.shape[1] < WIDTH:
            raise ValueError(f'Unexpected shape for {path}: {arr.shape}')
        vol = arr[:, :WIDTH].reshape(DEPTH, HEIGHT, WIDTH).astype(np.float32)
    else:
        raise ValueError(f'Unexpected shape for {path}: {arr.shape}')
    return vol
...
def parse_args():
    ...
    parser.add_argument('--input-dir', type=Path, default=here / '2025062905')
    parser.add_argument('--output-dir', type=Path, default=here / '2025062905_fill')
    parser.add_argument('--frames', type=int, default=10)
    return parser.parse_args()

def raw_data_files(input_dir):
    return sorted((path for path in input_dir.rglob('*_data.npy') if not path.name.endswith('_data2D.npy') and (not path.name.endswith('_data_pre2D.npy'))))
...
def complete_volume_with_mask(vol_raw, mask_raw):
    ...
    for Lk in LOW_LEVELS:
        ...
        if obs_count > 0 and filled_count > 0:
            ...
            if filled_mean != 0.0:
                scalar = obs_mean / max(filled_mean, 1e-06)
...
def main():
    ...
    data_files = raw_data_files(args.input_dir)[:args.frames]
    mask_files = sorted(args.input_dir.rglob('*_mask.npy'))[:args.frames]
    if len(data_files) != args.frames or len(mask_files) != args.frames:
        raise ValueError(f'{args.input_dir} requires {args.frames} data/mask pairs; found {len(data_files)} data and {len(mask_files)} masks')
    for data_path, mask_path in zip(data_files, mask_files):
        data_stamp = data_path.name.removesuffix('_data.npy')
        mask_stamp = mask_path.name.removesuffix('_mask.npy')
        if data_stamp != mask_stamp:
            raise ValueError(f'Timestamp mismatch: {data_path} vs {mask_path}')
        vol_raw = load_frame(data_path)
        mask_raw = load_frame(mask_path)
        filled_raw = complete_volume_with_mask(vol_raw, mask_raw)
        data_output = args.output_dir / data_path.name
        mask_output = args.output_dir / mask_path.name
        np.save(data_output, filled_raw.astype(np.float32))
        np.save(mask_output, mask_raw.astype(np.float32))
