...
class ConvGRUCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, n_layer=1):
        ...
        self.conv_gates = nn.ModuleList([nn.Conv2d(in_channels=input_dim + hidden_dim if i == 0 else hidden_dim * 2, out_channels=2 * self.hidden_dim, kernel_size=kernel_size, padding=self.padding) for i in range(n_layer)])
        self.conv_cans = nn.ModuleList([nn.Conv2d(in_channels=input_dim + hidden_dim if i == 0 else hidden_dim * 2, out_channels=self.hidden_dim, kernel_size=kernel_size, padding=self.padding) for i in range(n_layer)])
...
class ContextNet(nn.Module):
    ...
    def __init__(self, dim, dim_mults=(1, 2, 4, 8), channels=1):
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([ResnetBlock(dim_in, dim_in), ConvGRUCell(dim_in, dim_in, 3, n_layer=1), Downsample(dim_in, dim_out) if not is_last else nn.Identity()]))
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        state_shape = (b, c, h, w)
        self.init_state(state_shape, x.device)
        x = self.init_conv(x)
        context = []
...
class Unet(nn.Module):
    ...
    def __init__(self, dim, T_in, T_out, dim_mults=(1, 2, 4, 8), self_condition=True, resnet_block_groups=8, learned_sinusoidal_cond=False, random_fourier_features=False, learned_sinusoidal_dim=16):
        super().__init__()
        self.channels = 2 * T_out
        ...
        self.frag_idx_mlp = nn.Sequential(SinusoidalPosEmb(dim), nn.Linear(fourier_dim, time_dim), nn.GELU(), nn.Linear(time_dim, time_dim))
        self.downs = nn.ModuleList([])
        self.ups = nn.ModuleList([])
        num_resolutions = len(in_out)
        for ind, (dim_in, dim_out) in enumerate(in_out):
            is_last = ind >= num_resolutions - 1
            self.downs.append(nn.ModuleList([block_klass(dim_in * 2, dim_in, time_emb_dim=time_dim * 2), block_klass(dim_in, dim_in, time_emb_dim=time_dim * 2), Residual(PreNorm(dim_in, TemporalAttention(dim_in))), Downsample(dim_in, dim_out) if not is_last else nn.Conv2d(dim_in, dim_out, 3, padding=1)]))
        mid_dim = dims[-1]
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            is_last = ind == len(in_out) - 1
            self.ups.append(nn.ModuleList([block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim * 2), block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim * 2), Residual(PreNorm(dim_out, TemporalAttention(dim_out))), Upsample(dim_out, dim_in) if not is_last else nn.Conv2d(dim_out, dim_in, 3, padding=1)]))
        self.out_dim = T_out
    ...
    def forward(self, x, time, cond=None, ctx=None, idx=None):
        ...
        B = t.shape[0]
        if idx is None:
            idx = torch.zeros(B, device=t.device)
        else:
            if not torch.is_tensor(idx):
                idx = torch.tensor(idx, device=t.device)
            idx = idx.to(device=t.device, dtype=torch.float32).view(-1)
            if idx.numel() == 1:
                idx = idx.expand(B)
            elif idx.numel() != B:
                raise RuntimeError(f'idx has {idx.numel()} elems, but batch B={B}')
...
class GaussianDiffusion(nn.Module):
    ...
    def __init__(self, model, ctx_net, *, timesteps=1000, sampling_timesteps=None, objective='pred_v', beta_schedule='sigmoid', schedule_fn_kwargs=dict(), ddim_sampling_eta=0.0, auto_normalize=True, offset_noise_strength=0.0, min_snr_loss_weight=False, min_snr_gamma=5):
        super().__init__()
        ...
        assert objective in {'pred_noise', 'pred_x0', 'pred_v'}, 'unsupported objective'
    ...
    def model_predictions_train(self, x, t, cond=None, ctx=None, idx=None, clip_x_start=False, rederive_pred_noise=False):
        model_output = self.model(x, t, cond=cond, ctx=ctx, idx=idx)
        maybe_clip = partial(torch.clamp, min=-1.0, max=1.0) if clip_x_start else identity
        if self.objective == 'pred_noise':
            pred_noise = model_output
    ...
    def ddim_sample(self, shape, cond=None, ctx=None, idx=None, return_all_timesteps=False):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step', disable=True):
            ...
            frames_pred = x_start * alpha_next.sqrt() + c * pred_noise + sigma * noise
            imgs.append(frames_pred)
        ret = frames_pred if not return_all_timesteps else torch.stack(imgs, dim=1)
        print('ddim finish')
        return ret
    ...
    @torch.no_grad()
    def sample(self, x_zmax_norm, x_pre_zmax_norm, return_all_timesteps=False):
        B, T_in, h, w = x_zmax_norm.shape
        T_out = x_pre_zmax_norm.shape[1]
        global_ctx = self.ctx_net(torch.cat((x_zmax_norm, x_pre_zmax_norm), dim=1))
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        preds = sample_fn((B, T_out, h, w), cond=x_pre_zmax_norm, ctx=global_ctx, idx=torch.tensor(1).to(x_zmax_norm.device), return_all_timesteps=return_all_timesteps)
        y_pred_norm = preds * 2 + x_pre_zmax_norm
        frames_pred = self.unnormalize(y_pred_norm)
        frames_pred = frames_pred.clamp(0, 1)
        return frames_pred

def get_model(img_channels=40, dim=64, dim_mults=(1, 2, 4, 8), T_in=10, T_out=30, timesteps=1000, sampling_timesteps=250, **kwargs):
    unet = Unet(dim=dim, T_in=T_in, T_out=T_out, dim_mults=dim_mults)
    context_net = ContextNet(dim=dim, dim_mults=dim_mults, channels=img_channels)
    diffusion = GaussianDiffusion(model=unet, ctx_net=context_net, timesteps=timesteps, sampling_timesteps=sampling_timesteps)
    return diffusion
