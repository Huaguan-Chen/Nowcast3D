...
| `Original Radar Data` | nationwide 3-D radar mosaic BIN files for conversion into regional NPY files with `Radardata2npy.py` |
...
With authorization from the participating institutions, both data preparation
and model training were carried out at the China Meteorological Administration
Earth System Modeling and Prediction Center and at the Tianjin Meteorological
Observatory.

To extend the radar reflectivity input from the original `24 x 256 x 256` to
`24 x 512 x 512` while minimizing performance degradation, the model
architecture was simplified to meet operational deployment requirements. The
system has been deployed and is currently in operational use in Tianjin,
Hebei, Guangxi, and other regions of China, as well as in Pakistan.
