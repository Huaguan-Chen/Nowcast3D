...
def physical_reflectivity_mean(*members):
    ...
    factors = 10.0 ** ((stack * 80.0) / 10.0)
    mean_factor = torch.clamp(factors.mean(dim=0), min=1e-12)
    return 10.0 * torch.log10(mean_factor) / 80.0
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(
        ...
        preds = sample_fn(
            (B, T_out, h, w), 
            cond = x_pre_zmax_norm, 
            ctx = global_ctx,
            idx = torch.tensor(1).to(x_zmax_norm.device), 
            return_all_timesteps = return_all_timesteps
            )

        # y_pred_norm = preds*0.7 + x_pre_zmax_norm
        y_pred_norm = preds*2 + x_pre_zmax_norm
                
        frames_pred = self.unnormalize(y_pred_norm)
        frames_pred = frames_pred.clamp(0,1)
        if threshold is not None:
            frames_pred = torch.where(
                frames_pred < threshold,
                torch.zeros_like(frames_pred),
                frames_pred,
            )
        if mask is not None:
            frames_pred = frames_pred * mask
        if physical_mean_with is not None:
            frames_pred = physical_reflectivity_mean(frames_pred, physical_mean_with)
            if mask is not None:
                frames_pred = frames_pred * mask
...
def q_sample(diffusion, x_start, t, noise):
